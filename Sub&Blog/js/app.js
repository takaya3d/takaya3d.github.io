...
const RoofCol = new THREE.MeshStandardMaterial({color: '#e8380d'});
...
class Trees {
    constructor() {
        ...
        const matTreeLeaves = new THREE.MeshPhongMaterial( { color:0xe8380d});
...
const floor = new THREE.Mesh(
    new THREE.PlaneGeometry(30, 30),
    new THREE.MeshStandardMaterial({color:0x004c83})
)
floor.geometry.setAttribute(
    'uv2',
    new THREE.Float32BufferAttribute(floor.geometry.attributes.uv.array, 2)
...
{
    positions[i] = (Math.random() - 0.5) * 38 // Math.random() - 0.5 to have a random value between -0.5 and +0.5
}

particlesGeometry.setAttribute('position', new THREE.BufferAttribute(positions, 3)) // Create the Three.js BufferAttribute and specify that each information is composed of 3 values

const particlesMaterial = new THREE.PointsMaterial()
particlesMaterial.size = 0.017
particlesMaterial.color = new THREE.Color('#fcf7b0');
 ...
 */
// Base camera
let camera = new THREE.PerspectiveCamera(103, sizes.width / sizes.height, 0.1, 100)
camera.position.x = 1
