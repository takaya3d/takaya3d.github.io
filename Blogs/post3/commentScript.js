...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (CMNT) {
        ...
        const uniq = {
            gglFormID: '1FAIpQLSfPyTHQrTj74259h7H3qWLsNlR0_0nA0Pv_XGlTLVrU7s-Zcw',
            splSheetID: '1GNZSUhf9kI8Kv3k6eEo89IFUwF0ybAMR_PUyPBKr04Q',
            urlKey: '1619777063',
            titleKey: '2125785488',
            nameKey: '1212463554',
            commentKey: '466798019',
            idKey: '979512415',
            replyIdKey: '1420743252'
        }

        const FORM = document.getElementById('form');

        //コメントに必要な要素をformに追加（スパム対策）
        FORM.innerHTML = '<input name="name" placeholder="名前"><textarea class="textarea_comment" name="comment" placeholder="コメント"  rows="4" maxlength="400"></textarea><br><div><button class="comment_button" type="button">送信</button><span class="anchor"></span><input type="email" name="email" style="display:none;" title="スパム用"></div>';
            ...
            if (response.ok) {
                ...
                if (csv) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        LIST.innerHTML += `<li id="${data[i][3]}" data-num="${i + 1}"><div>${i + 1}.<b>${escapeHTML(data[i][1])}</b><small>${data[i][0]}</small>  <a href="#form">返信</a></div>${reply}<pre>${escapeHTML(data[i][2])}</pre><br></li>`;
                    }
